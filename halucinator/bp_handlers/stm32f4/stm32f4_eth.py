...
class STM32F4Ethernet(BPHandler):
    ...
    def write_phy(self, qemu, bp_addr):
        reg = qemu.get_reg("r1") & 0xFFFF
        self.phy_registers[reg] = qemu.read_memory(qemu.get_reg("r2"), 4, 1)
        return True, 0

    @bp_handler(['HAL_ETH_ReadPHYRegister'])
    def read_phy(self, qemu, bp_addr):
        reg = qemu.get_reg("r1") & 0xFFFF
        qemu.write_memory(qemu.get_reg("r2"), 4, self.phy_registers[reg], 1)
        return True, 0

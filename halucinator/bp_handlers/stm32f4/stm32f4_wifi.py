...
class STM32F4Wifi(BPHandler):
    ...
    def wifi_socket_server_write(self, qemu, bp_addr):
        '''
            This version only supports the TCP/IP layer, do nothing
        '''
        length = qemu.get_reg("r0")
        data = qemu.read_memory(qemu.get_reg("r1"), 1, length, raw=True)
    ...
    def wifi_tim_handler(self, qemu, bp_addr):
        ...
        if self.wifi_state == WIFI_OFF and self.model.sock is None:
            ...
            qemu.set_reg("pc", qemu.avatar.callables['ind_wifi_connected'])
        # If a client has connected, and we don't know that yet, call the callback and set the mode.
            ...
            qemu.set_reg("pc", qemu.avatar.callables['ind_socket_server_client_joined'])
            ...
            if data is not None:
                ...
                qemu.set_reg("r0", 0)  # Socket id
                qemu.set_reg("r1", RX_DATA_BUF)  # buffer
                qemu.set_reg("r2", len(data))  # length
                # Chunk size.  TODO: What is this, even?
                qemu.set_reg("r3", len(data))
                qemu.set_reg("pc", qemu.avatar.callables['ind_wifi_socket_data_received'])
                ...
                self.wifi_state = WIFI_IDLE
                qemu.set_reg("pc", qemu.avatar.callables['ind_socket_server_client_left'])
        else:
            # No callback needed
            return True, 0
